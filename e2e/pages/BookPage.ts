...
export default class BookPage {
  ...
  async createBook(book) {
    ...
    await this.page.getByLabel('Released').fill(book.year);
  ...
  async editBook(dialog: Locator, newBook) {
    await expect(this.page).toHaveURL(/\/books\/[a-z0-9-]+/);
    ...
    await dialog.getByLabel('Released').fill(newBook.year);
  ...
  async verifyListItem(listItem: Locator, book) {
