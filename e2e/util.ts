...
export const generateBookStatus = (): string =>
  ['reading', 'completed', 'on-hold', 'dropped', 'planned'].at(Math.floor(Math.random() * 5))!;

export const generateMovieStatus = (): string =>
...
export const generateBook = (status?: string) => ({
  title: generateWords(),
  author: faker.person.fullName(),
  publisher: generateWords(),
...
export const generateMovie = (status?: string) => ({
  title: generateWords(),
  studio: faker.person.fullName(),
  director: faker.person.fullName(),
  writer: faker.person.fullName(),
