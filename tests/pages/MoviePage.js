...
export default class MoviePage {
  ...
  async createMovie(movie) {
    ...
    await this.page.getByLabel('Year').fill(movie.year);
  ...
  async editMovie(dialog, newMovie) {
    await expect(this.page).toHaveURL(/\/movies\/[a-z0-9-]+/);
    ...
    await dialog.getByLabel('Year').fill(newMovie.year);
  ...
  async verifyListItem(listItem, movie) {
